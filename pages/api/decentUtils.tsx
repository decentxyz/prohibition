...
import { parseAbi, Chain } from 'viem';
...
const debug = false; // enable (true) or disable (false) debug logging
...
export async function getTransactionStatus(chainId: number, txHash: string) {
    ...
    try {
        ...
        if (debug) { console.log('Transaction status:', data.status, ' Destination chain TX hash: ', data.transaction.dstTx.fast.transactionHash) };
